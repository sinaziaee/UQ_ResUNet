...
import datetime
import time
from utils import MultiChannelMaskTransform, PolyLRScheduler, validate_epoch, combined_loss, ema_update
...
def generate_pseudo_labels(teacher_model, unlabeled_loader, device):
    ...
    with torch.no_grad():
        for images in tqdm(unlabeled_loader):
...
def main():
    ...
    unlabeled_dataset = KitsDataset(images_dir=unlabeled_images_dir, segmentations_dir=None, transform=transform)
    unlabeled_loader = DataLoader(unlabeled_dataset, batch_size=configs.BATCH_SIZE, shuffle=False, num_workers=4)
    ...
    student_model = ResidualUNet(in_channels=1, num_classes=4).to(device)
